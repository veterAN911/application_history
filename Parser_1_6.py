...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_2.setText(_translate("MainWindow", "Версия 1.6"))
    ...
    def applicat(self):
        ...
        if self.plainTextEdit.toPlainText() == '':
            ...
            try:
                ...
                list_clients = {"Прайс", "Fix", "Верный", "О'КЕЙ", "ВИНЛАБ", "ВИКТОРИЯ",'Азбука Вкуса'}
                for i in range(len(telo)):
                    applications_telo += [[applications[::-1][i]] + telo[::-1][i]]
                    for name in applications_telo:
                        if "Прайс" not in  name[2] and "Fix" not in  name[2] and "Верный" not in  name[2] and  "О'КЕЙ" not in  name[2] and "ВИНЛАБ" not in  name[2] and "ВИКТОРИЯ" not in  name[2] and "Азбука Вкуса" not in  name[2]: 
                            name[2] = 'Fix Price'
                        if "Прайс" in name[2]: name[2] = 'Fix Price'
                        if "Fix" in name[2]: name[2] = 'Fix Price'
                        ...
                        name[1] = name[1].split(' ')[0] # Обработка имени
